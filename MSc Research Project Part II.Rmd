...
Some comments based on the set-up: When considering different thresholds of compliance in Cases I-IV, we do not expect the values of the Per-Protocol and IV estimates to differ since they remain unaffected by the threshold selection; they only depend on the values of D. However, the true treatment effect in compliers is expected to change in each case. So, we are interested to see if using a specific threshold will yield estimates closer to the truth. 
...
We will test all 4 Cases for $\delta=0.5$ first and then for $\delta=1$, i.e. severe and mild dependence on $Y(0)$ respectively.

We begin with Scenario A. We will implement it for all 4 cases (Cases I-IV). The code remains exactly the same with the addition of the condition for Scenario A which is highlighted inside the code. Below is the implementation for Scenario A, Case I. First we will implement all cases for $\delta=0.5$.
```{r}
simfun <- function(i, n=350, mean0 = 74, mean1 = 69, sd = 1, prob = 0.5){
  #Data Generating Mechanism (DGM)
  dt <- data.frame(
...
Now we consider all 4 cases for $\delta=1$.
```{r}
simfun <- function(i, n=350, mean0 = 74, mean1 = 69, sd = 1, prob = 0.5){
  #Data Generating Mechanism (DGM)
  dt <- data.frame(
...
Conclusive comments: For Scenario A, where participants with good conditions (low DBP) would never receive the treatment, for both cases (I-IV), where we allow non-compliers in both treatment groups (both always-takers and never-takers), both the Per-Protocol and the IV analyses yield biased estimates for both the ATE and the CACE for either value of $\delta$. Here we observe that the Per-Protocol and IV estimates do not change when considering different thresholds since they are unaffected by the choice of threshold. However, we see that the true treatment effect in compliers changes with each threshold value.


We proceed with Scenario B. We will implement it for all 4 cases (Cases I-IV). The code remains exactly the same with the addition of the condition for Scenario B which is highlighted inside the code. First we test the 4 cases for $\delta=0.5$. Below is the implementation for Scenario B, Case I. 
```{r}
simfun <- function(i, n=350, mean0 = 74, mean1 = 69, sd = 1, prob = 0.5){
  #Data Generating Mechanism (DGM)
  dt <- data.frame(
...
cat("IV Coverage:", IV_coverage, "\n")
```
Now we test all 4 cases for $\delta=1$.
```{r}
simfun <- function(i, n=350, mean0 = 74, mean1 = 69, sd = 1, prob = 0.5){
  #Data Generating Mechanism (DGM)
  dt <- data.frame(
...
We proceed with Scenario C. We will implement it for all 4 cases (Cases I-IV). The code remains exactly the same with the addition of the condition for Scenario C which is highlighted inside the code. Below is the implementation for Scenario C, Case I. First we set $\delta=0.5$ for all 4 cases.
```{r}
simfun <- function(i, n=350, mean0 = 74, mean1 = 69, sd = 1, prob = 0.5){
  #Data Generating Mechanism (DGM)
  dt <- data.frame(
...
cat("IV Coverage:", IV_coverage, "\n")
```
Conclusive comments: For Scenario A, where participants with good conditions (low DBP) would never receive the treatment, for both cases (I-IV), where we allow non-compliers only in the treatment group (only never-takers), both the Per-Protocol and the IV analyses yield biased estimates for CACE, while the IV also yields biased estimates for the ATE as well for either value of $\delta$. However, the Per-Protocol analysis yields unbiased estimates (for all 4 cases) for the ATE regardless of the value of $\delta$.
...
cat("IV Coverage:", IV_coverage, "\n")
```
Conclusive comments: For Scenario B, where participants with bad conditions (high DBP) would never receive the treatment, for both cases (I-IV), where we allow non-compliers only in the treatment group (only never-takers), both the Per-Protocol and the IV analyses yield biased estimates for CACE, while the IV also yields biased estimates for the ATE as well for either value of $\delta$. However, the Per-Protocol analysis yields unbiased estimates (for all 4 cases) for the ATE, regardless of the value of $\delta$ considered.


In summary, for partial compliance, when non-compliance occurs at random, both the Per-Protocol and the IV analyses will yield unbiased estimates, regardless of the compliance threshold or the type of non-compliers. On the other hand, when non-compliance does not occur at random, none of the two methods yields an unbiased estimate for any of the six scenarios considered regardless of the compliance threshold, the type of non-compliers or the degree of dependence on $Y(0)$ (value of $\delta$). The important features we notice are for Scenarios E and F. Specifically, for Scenario E, for $\delta=1$, the IV analysis yields approximately unbiased estimates for the CACE for all cases (I-IV). Also, for Scenario F, for $\delta=0.5$, the IV analysis yields approximately unbiased estimates for the CACE for all cases (I-IV) and for $\delta=1$, it yields unbiased estimates for the CACE for all cases (I-IV).


We will now re-run all the simulations above but this time we will set the treatment effect equal to $0$, i.e. $\mu_0=\mu_1=74$. We begin with random non-compliance and Case I.
```{r randompartial3}
...
We will test all Scenarios for $\delta=0.5$ and $\delta=1$, i.e. severe or mild dependence on $Y(0)$ respectively (Bang & Davis 2007).

We begin with Scenario A. We will implement it for all 4 cases (Cases I-IV). The code remains exactly the same with the addition of the condition for Scenario A which is highlighted inside the code. Below is the implementation for Scenario A, Case I. We first set $\delta=0.5$.
```{r}
simfun <- function(i, n=350, mean0 = 74, mean1 = 74, sd = 1, prob = 0.5){
  #Data Generating Mechanism (DGM)
  dt <- data.frame(
...
Conclusive comments: For Scenario E, where participants with bad conditions would always receive the treatment and participants with good conditions would always reject the treatment, for both cases (I-IV), where we allow non-compliers in both treatment groups (both always-takers and never-takers), both the Per-Protocol and the IV analyses yield biased estimates for both the ATE and the CACE for $\delta=0.5$. For the Per-Protocol estimates, the same thing is observed for $\delta=1$. However, for the IV estimates, for $\delta=1$, we observe that they are approximately unbiased for the CACE for all 4 cases (I-IV).
...
Conclusive comments: For Scenario F, where participants with bad conditions would never receive the treatment and participants with good conditions would always receive the treatment, for both cases (I-IV), where we allow non-compliers in both treatment groups (both always-takers and never-takers), the Per-Protocol analysis yields biased estimates for both the ATE and the CACE, for either value of $\delta$. On the other hand, the IV analysis yields approximately unbiased estimates (for all cases I-IV) for the CACE when considering $\delta=0.5$ and unbiased estimates when considering $\delta=1$.
...
We now proceed to the non-random non-compliance cases. The only plausible of the 6 Scenarios, when we allow only for never-takers, are Scenarios A and B. We will implement all 4 cases for these two Scenarios and this will conclude our experimentation. We begin with Scenario A, Case I. We first set $\delta=0.5$.
```{r}
simfun <- function(i, n=350, mean0 = 74, mean1 = 74, sd = 1, prob = 0.5){
  #Data Generating Mechanism (DGM)
  dt <- data.frame(
...
In summary, the results obtain are identical to those obtained when considering the true treatment effect to be $5$.
